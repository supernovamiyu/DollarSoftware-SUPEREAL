...
// Modificar la función mostrarProductosDestacados para hacer que todo el contenedor sea clickeable
function mostrarProductosDestacados() {
    fetch("http://localhost:3000/products/destacados")
        ...
        .then((data) => {
            ...
            const contenidoProductos = productosLimitados
                .map((producto) => {
                    return `
                <div class="producto-contenedor" onclick="mostrarPantallaProductoIndividual(event, '${producto.id_productos}')" data-id="${producto.id_productos}">
                    <img src="${producto.imagen_url}" alt="Imagen del producto" width="50%" height="auto">
                    <h3>${producto.nombre_producto}</h3>
                    <p>Precio: <br> $${producto.precio}</p>
                    <div class="botones-producto">
                        <button class="comprar" onclick="agregarAlCarrito('${producto.id_productos}', event)" data-id="${producto.id_productos}" data-unidades-disponibles="${producto.unidades_disponibles}">Añadir al carrito</button>
                    </div>
                </div>
                `
                })
            ...
            // Agregar event listeners para evitar que el clic en el botón propague al contenedor
...
// Modificar la función mostrarProductos para hacer que todo el contenedor sea clickeable
...
// Modificar la función mostrarResultadosDeBusqueda para hacer que todo el contenedor sea clickeable
...
function mostrarPantallaProductoIndividual(event, id_productos) {
    ...
    // Actualizar la URL
    const url = `/producto/${id_productos}`
    actualizarURL(url)
