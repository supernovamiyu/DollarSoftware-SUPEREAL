...
function cerrarSesion() {
    ...
    try {
        // Intentar usar la vista para mostrar la plantilla de inicio
        const vista = new Vista()
        vista.mostrarPlantilla("plantilla-inicio", "container-principal")
        console.log("Redirección usando vista.mostrarPlantilla()")

        // Actualizar la URL si es necesario
        if (typeof actualizarURL === "function") {
            actualizarURL("/")
        }

        // Cargar productos destacados si existe la función
        if (typeof mostrarProductosDestacados === "function") {
        ...
        try {
            ...
            console.error("Error al redirigir usando window.location:", finalError)
            alert("No se pudo redirigir a la página de inicio. Por favor, recarga la página manualmente.")
        }
    }
}
