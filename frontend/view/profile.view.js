...
class ProfileView extends BaseView {
    ...
    showProfileSection(sectionId) {
        ...
        if (!section) {
            ...
            // Agregar la sección al contenedor de contenido del perfil
                        ...
                        <table class="tabla-de-pedidos">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>${new Date(order.fecha_pedido || order.fecha).toLocaleDateString()}</td>
                                    <td>${order.productos ? order.productos.length : 0}</td>
                                    <td>$${(order.total || 0).toFixed(2)}</td>
                                    <td>${this.getOrderStatusBadge(order.estado || order.status || 'pending')}</td>
                                    <td>
                                        <button class="btn btn-primary view-order-btn" data-order-id="${order.id_pedido || order.id}">
                                            Ver Detalles
                                        </button>
                                    </td>
                                </tr>
    ...
    setupProfileButtons(buttonClickHandler) {
        // Configurar botones principales del perfil
    ...
    showOrderDetailsModal(order) {
        ...
        // Llenar con los datos del pedido
        document.getElementById("orderDetailId").textContent = order.id_pedido || order.id;
        document.getElementById("orderDetailDate").textContent = new Date(order.fecha_pedido || order.date).toLocaleDateString();
        document.getElementById("orderDetailStatus").innerHTML = this.getOrderStatusBadge(order.estado || order.status);
        document.getElementById("orderDetailAddress").textContent = order.direccion_entrega || order.shipping_address || "No especificada";
        
        const productsList = document.getElementById("orderProductsList");
        productsList.innerHTML = (order.productos || order.products || []).map(product => `
